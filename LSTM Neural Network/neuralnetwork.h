...
public:
	...
	// Embedding Layer class to map tokens (e.g. "for", "int") to dense vectors
	class EmbeddingLayer {
	public:
		int vocab_size;	// Number of unique tokens.
		int embedding_dim;	// Size of the embedding vectors.
		Matrix embedding_matrix;	// Matrix to store the embeddings.

		EmbeddingLayer(int vocab_size, int embedding_dim)
			: vocab_size(vocab_size),
			embedding_dim(embedding_dim) {
			// Initialize embedding matrix with random values.
			embedding_matrix = Matrix(vocab_size, embedding_dim);
			embedding_matrix = embedding_matrix.applyFunction(
				[](const float& f) {
					return ((float)rand() / RAND_MAX) * 2.0f - 1.0f;	// Random values between -1 and 1.
				}
			);
		}

		// Forward pass: Convert token indices to embeddings.
		Matrix forward(const std::vector<int>& token_indices) {
			Matrix output(token_indices.size(), embedding_dim);
			for (size_t i = 0; i < token_indices.size(); i++) {
				for (int j = 0; j < embedding_dim; j++) {
					output.at(j, i) = embedding_matrix.at(j, token_indices[i]);
				}
			}
			return output;
		}
